...
%macro extract_clm(dummy);
	if missing(BENE_ID) then DELETE;
	if TYPE_CLM_CD="1" AND MAX_TOS in (1,8,12,11,16);
	if DIAG_CD_1 in ("493", "4930", "49300", "49301", "49302", "4931", "49310", "49311", "49312", "4932", 
	 ...
	 if DIAG_CD_1 in ("401","4010","4011", "4019", "402", "4020", "40200", "4021", "40210", "4029", "40290", 
	 ...
	 if DIAG_CD_1 in ("140", "1400", "1401", "1403", "1404", "1405", "1406", "1408", "1409", "141", "1410", "1411", 
	 ...
	 if DIAG_CD_1 in ("290", "2900", "2901", "29010", "29011", "29012", "29013", "2902", "29020", "29021", "2903", "2904", 
	 ...
	 if DIAG_CD_1 in ("630", "631", "6310", "6318", "632", "633", "6330", "63300", "63301", "6331", "63310", "63311", "6332", 
	 ...
	 if DIAG_CD_1 in ("24900", "24901", "24980", "24981", "24990", "24991", "250", "2500", "25000", "25002", "2507", "25070", 
...
data work.ot_sub_&state (keep=bene_id state_cd svc_dx service_type MAX_TOS MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_DAYS CLM_CNT);
	%if &state=CA | &state=NY | &state=TX %THEN %DO;
    		set stateot.Maxdata_&state._ot_2012_001  stateot.Maxdata_&state._ot_2012_002 (keep=BENE_ID STATE_CD DIAG_CD_1 MAX_TOS PLC_OF_SRVC_CD MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_BGN_DT SRVC_END_DT);
    		%END;
    		%ELSE %DO;
    		set stateot.Maxdata_&state._ot_2012 (keep=BENE_ID STATE_CD DIAG_CD_1 MAX_TOS PLC_OF_SRVC_CD MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_BGN_DT SRVC_END_DT);
    		%END;
%extract_clm;
	if PLC_OF_SRVC_CD = 23 then service_type="EMERG_ROOM";
		else if MAX_TOS in (8,12) then service_type="PHYS_CLIN";
		else if MAX_TOS=11 then service_type="HOSP_OPT";
		else if MAX_TOS=16 then service_type="RX_DRUG";
		else if MAX_TOS=1 then service_type="IP_HOSP";
run;
