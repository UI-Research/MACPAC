...
%let indata=data.maxdata_ps_2012; %let outdata=personlevel;
	...
	data &outdata.;
		...
		length age_servicetype $16.;
			...
			latest_servicetype = coalescec(servicetype_12,servicetype_11,servicetype_10,servicetype__9,servicetype__8,servicetype__7,servicetype__6,servicetype__5,servicetype__4,
											servicetype__3,servicetype__2,servicetype__1);
			if age_cat=1 then do; age_servicetype = catx('_','child',latest_servicetype); end;
			else if age_cat=2 then do; age_servicetype = catx('_','adult',latest_servicetype); end;
			else if age_cat=3 then do; age_servicetype = catx('_','senior',latest_servicetype);end;
...
proc freq data=space.id_pop_&space_name.;
	tables age_servicetype;
run;
proc sql noprint;
