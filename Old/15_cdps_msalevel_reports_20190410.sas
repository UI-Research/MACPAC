...
proc sql;
	...
	select *
	from space.bene_svcdx_svctye_cdpsclaims a left join space.max_cdps_2012 b/*input data file from 05_subset_claims_byDX*/
	on a.bene_id = b.bene_id
	where b.age_cat ne 3 and a.bene_id ne '';
quit; 

%let indata_max = no_seniors; 
...
/********************************************************/
proc contents data=&indata_max;run;
...
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "DIAB");
quit;
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "HYPT");
quit;
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "CANC");
quit;
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "PREG");
quit;
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "PSYC");
quit;
proc sql;
	...
	select *
	from &indata_max.
	where bene_id in (select distinct bene_id from &indata_max. where svc_dx = "ASTH");
quit;
...
%macro collapse(in_data,collapse_on, orig_data);
	...
	*add ahrf/msa data to claims;
	...
	proc univariate data=&in_data._msas noprint;
		...
		pctlpts = 99.5
		pctlpre=mdcd_spd_p res_spd_p inpt_spd_p physclin_spd_p rx_spd_p otpt_spd_p nf_spd_p;
	run;
	...
	proc sql;
	  create table &in_data._msas_spdTC AS
		select a.*, 
			...
			case when a.FFS_PYMT_AMT_16>B.rx_spd_p99_5 then B.rx_spd_p99_5 else a.FFS_PYMT_AMT_16 end as rx_spdTC,
...
%collapse(in_data=dia2l_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
%collapse(in_data=carel_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
%collapse(in_data=canl_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
%collapse(in_data=prgcmp_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
%collapse(in_data=psyl_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
%collapse(in_data=pula_cdps_benes,collapse_on=st_msa, orig_data=ahrf_aggre);
proc contents data=space.prgmp_cdps_benes_collapsed;run;
proc export data=space.dia2l_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_dia2l_nomasboe_noage.csv' dbms=csv replace;
run;
proc export data=space.carel_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_carel_nomasboe_noage.csv' dbms=csv replace;
run;
proc export data=space.canl_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_canl_nomasboe_noage.csv' dbms=csv replace;
run;
proc export data=space.prgcmp_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_prgcmp_nomasboe_noage.csv' dbms=csv replace;
run;
proc export data=space.psyl_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_psyl_nomasboe_noage.csv' dbms=csv replace;
run;
proc export data=space.pula_cdps_benes_collapsed
   outfile='P:\MCD-SPVR\data\NO_PII\msa_pula_nomasboe_noage.csv' dbms=csv replace;
run;
