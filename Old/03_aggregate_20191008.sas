...
%let indata_max = space.id_pop_09oct2019; /*input data file from 01_studypop_analyticfile*/
%let year = 2012;
...
%macro hrr;
	* HRR-ZIP mapping file to merge wtih MAX data;
	...
	data hrr_zip_f;
	  ...
	  * unique HRR-state ID;
	  hrr_state=catx('-',hrrnum_c,hrrstate);
	...
	* keep only records in MAX - drop obs only in ahrf - format hrr_state & recode ahrf merge flag to 1/0;
	...
	proc freq data=max_zip_join;
		tables hrrnum zip_code zipcode/list missing;
		format hrrnum missing_num. zip_code zipcode missing_zip.;
	run;

	proc sql;
		title Obs without HRR Zip matches;
		select hrrnum,count(hrrnum) as number_missing
		from max_zip_join
		where hrrnum = .
		group by hrrnum;
	quit;
...
%macro collapse (indata,collapse_on,outdata);
	proc sql;
		create table &outdata. as
			...
			sum(case when full_benf = 99 then 1 else 0 end) as tot_partial_benf,
			sum(case when full_benf = 1 then 1 else 0 end) as tot_full_benf,
			sum(case when full_benf = 0 then 1 else 0 end) as tot_no_benf,
			sum(case when full_benf = 1 then sum(
				(case when partial_benefits_1 = 1 then 1 else 0 end),
				(case when partial_benefits_2 = 1 then 1 else 0 end),
				(case when partial_benefits_3 = 1 then 1 else 0 end),
				(case when partial_benefits_4 = 1 then 1 else 0 end),
				(case when partial_benefits_5 = 1 then 1 else 0 end),
				(case when partial_benefits_6 = 1 then 1 else 0 end),
				(case when partial_benefits_7 = 1 then 1 else 0 end),
				(case when partial_benefits_8 = 1 then 1 else 0 end),
				(case when partial_benefits_9 = 1 then 1 else 0 end),
				(case when partial_benefits_10 = 1 then 1 else 0 end),
				(case when partial_benefits_11 = 1 then 1 else 0 end),
				(case when partial_benefits_12 = 1 then 1 else 0 end)) else 0 end) as partial_benf_mo,
			sum(case when full_benf = 1 then TOT_MDCD_PYMT_AMT else 0 end) as spending,
			sum(TOT_MDCD_PYMT_AMT) as all_spending,
			sum(case when full_benf = 1 then mo_dual else 0 end) as dual_mon, 
			sum(case when full_benf = 1 then mo_mc else 0 end) as mc_mon, 
			sum(case when full_benf = 1 then mo_dsbl else 0 end) as dis_mon, 
			sum(case when full_benf = 1 then mo_ltss else 0 end) as ltss_mon,
			sum(case when full_benf = 1 then EL_ELGBLTY_MO_CNT else 0 end) as elg_mon, 
			sum(case when full_benf = 1 then cell_n else 0 end) as cell_n,
			sum(case when full_benf = 1 then d_servicetype_n else 0 end) as d_servicetype_n, 
			sum(case when full_benf = 1 then died_n else 0 end) as died_n,
			sum(case when full_benf = 1 then mas_cash else 0 end) as mas_cash_n, 
			sum(case when full_benf = 1 then mas_cash else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as mas_cash,
			sum(case when full_benf = 1 then mas_mn else 0 end) as mas_mn_n, 
			sum(case when full_benf = 1 then mas_mn else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as mas_mn, 
			sum(case when full_benf = 1 then mas_pov else 0 end) as mas_pov_n, 
			sum(case when full_benf = 1 then mas_pov else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as mas_pov, 
			sum(case when full_benf = 1 then mas_1115 else 0 end) as mas_1115_n,
			sum(case when full_benf = 1 then mas_1115 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as mas_1115,
			sum(case when full_benf = 1 then mas_oth else 0 end) as mas_oth_n, 
			sum(case when full_benf = 1 then mas_oth else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as mas_oth, 
			sum(case when full_benf = 1 then boe_aged else 0 end) as boe_aged_n, 
			sum(case when full_benf = 1 then boe_aged else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_aged, 
			sum(case when full_benf = 1 then boe_disabled else 0 end) as boe_disabled_n, 
			sum(case when full_benf = 1 then boe_disabled else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_disabled, 
			sum(case when full_benf = 1 then boe_child else 0 end) as boe_child_n, 
			sum(case when full_benf = 1 then boe_child else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_child, 
			sum(case when full_benf = 1 then boe_adult else 0 end) as boe_adult_n, 
			sum(case when full_benf = 1 then boe_adult else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_adult, 
			sum(case when full_benf = 1 then boe_uchild else 0 end) as boe_uchild_n, 
			sum(case when full_benf = 1 then boe_uchild else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_uchild, 
			sum(case when full_benf = 1 then boe_uadult else 0 end) as boe_uadult_n, 
			sum(case when full_benf = 1 then boe_uadult else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_uadult, 
			sum(case when full_benf = 1 then boe_fchild else 0 end) as boe_fchild_n,
			sum(case when full_benf = 1 then boe_fchild else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as boe_fchild,
			sum(case when full_benf = 1 then male else 0 end) as male_n, 
			sum(case when full_benf = 1 then male else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as male, 
			sum(case when full_benf = 1 then age_0 else 0 end) as _0_n, 
			sum(case when full_benf = 1 then age_0 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _0, 
			sum(case when full_benf = 1 then age_1_5 else 0 end) as _1_5_n, 
			sum(case when full_benf = 1 then age_1_5 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _1_5, 
			sum(case when full_benf = 1 then age_6_18 else 0 end) as _6_18_n, 
			sum(case when full_benf = 1 then age_6_18 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _6_18, 
			sum(case when full_benf = 1 then age_19_44 else 0 end) as _19_44_n, 
			sum(case when full_benf = 1 then age_19_44 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _19_44, 
			sum(case when full_benf = 1 then age_45_64 else 0 end) as _45_64_n, 
			sum(case when full_benf = 1 then age_45_64 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _45_64, 
			sum(case when full_benf = 1 then age_65_84 else 0 end) as _65_84_n, 
			sum(case when full_benf = 1 then age_65_84 else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _65_84, 
			sum(case when full_benf = 1 then age_85p else 0 end) as _85p_n, 
			sum(case when full_benf = 1 then age_85p else 0 end)/sum(case when full_benf = 1 then cell_n else 0 end) as _85p,
			mean(case when full_benf = 1 then CDPS_SCORE else 0 end) as cdps,
			mean(case when full_benf = 1 then NOCDPS else 0 end) as no_cdps_conds,
			mean(case when full_benf = 1 then pspend_i else 0 end) as pred_mcd_spd
			from &indata. 
...
proc freq data=msa_collapse;
	tables tot_full_benf*partial_benf_mo/list missing;
run;
...
proc sql;
	...
	select msa.*, wag.WORK as wageind_WORK, wag.PE as wageind_PE, wag.MPE as wageind_MPE
	from msa_arhfvars msa left join data.wageindex_2012 wag
	on msa.st_msa = catx('-',wag.STATE, wag.MSA_NO_);
quit;
	...
	*mark too-small cells missing -skipping this for now; 
	data &outdata. ;
		...
		label 	
			...
			all_spending = "Total Annual Spending for all beneficiaries (full and partial benefits)"
			tot_full_benf = "Total number of beneficiaries with full benefits in last observed month"
			tot_no_benf = "Total number of beneficiaries with no benefits in last observed month"
			tot_partial_benf = "Total number of beneficiaries with partial benefits in last observed month"
			partial_benf_mo = "Number of person months with partial benefits" 
			;
			/*
...
%mend;
%get_stats(indata=max_2012_msa_join,indata_collapsed=msa_arhfvars_wageind,orig_data=ahrf_msa_xwalk, collapsevar=st_msa,outdata=out.msa_2012_&space_name.);
...
%let msa = out.msa_2012_&space_name.;
*%let hrr = out.hrr_2012_&space_name.;
...
proc contents data=out.msa_2012_09oct2019;run;
/*
&dum_tab.;
ods excel options(sheet_name="hrr" sheet_interval="none");
data hrr_num(keep=_NUMERIC_) hrr_char(keep=_CHARACTER_);
    set &hrr.;
run;
proc odstext;
	  p "&hrr. descriptive statistics" / style=[color=red font_weight=bold];
run;
proc means stackodsoutput data=hrr_num n mean std min max nmiss;
	title "&hrr. descriptive statistics for numeric variables";
run;
proc odstext;
	  p "Show cells marked .S in &hrr." / style=[font_weight=bold];
run;
proc freq data=&hrr;
	title "Show cells marked .S in &hrr.";
	tables spd_p50 res_spd_max/  missing;
	format spd_p50 res_spd_max missing_S.;
run;

proc freq data=hrr_char;
	title "&hrr. descriptive statistics for character variables";
run;e
*/
ods excel close;
/*export a stata copy;
