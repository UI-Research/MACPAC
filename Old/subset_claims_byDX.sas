...
%macro extract_clm;
	...
	if TYPE_CLM_CD=1 AND MAX_TOS in ('1','01','08','8','12','11','16');
	if DIAG_CD_1 in ('493', '4930', '49300', '49301', '49302', '4931', '49310', '49311', '49312', '4932', 
			'49320', '49321', '49322', '4938', '49381', '49382', '4939', '49390', '49391', '49392') then svc_dx="ASTH";
	else if DIAG_CD_1 in ('401','4010','4011', '4019', '402', '4020', '40200', '4021', '40210', '4029', '40290', 
			'403', '4030', '40300', '4031', '40310', '4039', '40390', '404', '4040', '40400', '4041', '40410', 
			'4049', '40490', '405', '4050', '40501', '40509', '4051', '40511', '40519', '4059', '40591', '40599') then svc_dx="HYPT";
	else if DIAG_CD_1 in ('140', '1400', '1401', '1403', '1404', '1405', '1406', '1408', '1409', '141', '1410', '1411', 
			...
			'1953', '1954', '1955', '1958', 'V524') then svc_dx="CANC";
	else if DIAG_CD_1 in ('290', '2900', '2901', '29010', '29011', '29012', '29013', '2902', '29020', '29021', '2903', '2904', 
			...
			'3098', '30982', '30983', '30989', '31081', '31089', '311', '3310', '3311', '3312') then svc_dx="PSYC";
	else if DIAG_CD_1 in ('630', '631', '6310', '6318', '632', '633', '6330', '63300', '63301', '6331', '63310', '63311', '6332', 
			...
			'V276', 'V277', 'V279') then svc_dx="PREG";
	else if DIAG_CD_1 in ('24900', '24901', '24980', '24981', '24990', '24991', '250', '2500', '25000', '25002', '2507', '25070', 
			'25071', '25072', '25073', '2508', '25080', '25081', '25082', '25083', '2509', '25090', '25091', '25092', 
			'25093', '3620', '36201', '36203', '36204', '36205') then svc_dx="DIAB";
	if  missing(svc_dx) then DELETE;
	CLM_CNT=1;
	SRVC_DAYS=SRVC_END_DT-SRVC_BGN_DT+1;
%mend extract_clm;

%macro subset_ot(state);
data work.ot_sub_&state (keep=bene_id state_cd svc_dx service_type MAX_TOS MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_DAYS CLM_CNT);
	%if &state=CA | &state=NY | &state=TX %THEN %DO;
    		set stateot.Maxdata_&state._ot_2012_001  stateot.Maxdata_&state._ot_2012_002 (keep=BENE_ID STATE_CD DIAG_CD_1 MAX_TOS PLC_OF_SRVC_CD MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_BGN DT SRVC_END_DT);
    		%END;
    		%ELSE %DO;
    		set stateot.Maxdata_&state._ot_2012 (keep=BENE_ID STATE_CD DIAG_CD_1 MAX_TOS PLC_OF_SRVC_CD MDCD_PYMT_AMT QTY_SRVC_UNITS SRVC_BGN DT SRVC_END_DT);
    		%END;
%extract_clm;
	if PLC_OF_SRVC_CD = '23' then service_type="EMERG_ROOM";
		else if MAX_TOS in ('08','12') then service_type="PHYS_CLIN";
		else if MAX_TOS='11' then service_type="HOSP_OPT";
		else if MAX_TOS='16' then service_type="RX_DRUG";
		else if MAX_TOS in('01','1') then service_type="IP_HOSP";
run;

data work.ip_sub_&state (keep=bene_id state_cd svc_dx service_type MAX_TOS MDCD_PYMT_AMT SRVC_DAYS CLM_CNT);
  set data.Maxdata_ip_2012 (keep=BENE_ID STATE_CD DIAG_CD_1 MAX_TOS MDCD_PYMT_AMT  SRVC_BGN DT SRVC_END_DT);
	if state_cd="&state.";
%extract_clm;
	 service_type="IP_HOSP";
run;

proc sql;
	CREATE TABLE work.sum_&state. AS
	 ( SELECT
	  bene_id, state, svc_dx, service_type, 
	  SUM(MDCD_PYMT_AMT) AS mdcd_pymt_amt,
	  SUM(QTY_SRVC_UNITS) AS QTY_SRVC_UNITS,
	  SUM(SRVC_DAYS) AS SRVC_DAYS,
	  SUM(CLM_CNT) AS CLM_CNT
	  FROM work.ot_sub_&state.
	  GROUP BY bene_id, state, svc_dx, service_type)
    CORR UNION
	 ( SELECT
	  bene_id, state, svc_dx, service_type, 
	  SUM(MDCD_PYMT_AMT) AS mdcd_pymt_amt,
