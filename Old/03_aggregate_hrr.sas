...
/*Log
proc printto print="P:\MCD-SPVR\log\03_aggregate_hrr_&sysdate..lst"
               log="P:\MCD-SPVR\log\03_aggregate_hrr_&sysdate..log" NEW;
run;*/
...
%mend;
proc freq data=max_zip_2012;
	table hrrnum*Rspend_i/list;
run;

proc sql;
	select count(distinct hrrnum), beds, md, urate, povrate
	from max_zip_2012;

	select count(distinct hrrnum), beds, md, urate, povrate, Rspend_i
	from max_zip_2012;
quit;
...
%let indata=max_zip_2012; %let indata_collapsed=hrr_arhfvars; %let orig_data=hrr_zip; %let collapsevar=hrrnum; %let outdata=hrr_2012_&space_name.;
	...
	run;
proc print data=spend_pctls_&collapsevar. (obs=10);run;
	tables res_spd_p;
run;
	proc univariate data=&indata. noprint;
		class &collapsevar. ;
	...
	proc sql;
	  create table &indata._c AS
		select a.&collapsevar., a.age_servicetype, a.TOT_MDCD_PYMT_AMT as mcd_spd, a.Rspend_i as res_mcd_spd, 
			((a.TOT_MDCD_PYMT_AMT>B.p99_5)*B.p99_5) as mcd_spd_TC,((A.Rspend_i>B.res_spd_p99_5)*B.spd_p99_5) as res_mcd_spd_TC 
...
proc freq data=fintab_&collapsevar._ac;
	tables spd_:;
run;
/*export a stata copy;
