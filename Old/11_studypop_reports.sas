...
/*******************************************************************************************************************/ 
/*	Purpose: Create a dataset with desired MAX variables. Fix invalid geographic information.
...
/******************************************************************************
%macro data_cleaning(indata=);
%let indata=person_level;
	/*Print summary stats out to excel
	data &indata._num (keep=_NUMERIC_) person_level_char (keep=_CHARACTER_);
		set &indata.;
	run;

	ods excel file="&report_folder.\personlevel_initial_summary.xlsx";
	ods excel options(sheet_name="Numeric Summary Stats" sheet_interval="none");
	proc means data=&indata._num stackodsoutput  maxdec=2 n nmiss mean std min max;
	...
	proc freq data=&indata._char nlevels;
		title "&indata. nlevels";
		tables _all_;
	...
	proc sql;
		...
		from dictionary.columns
		where libname="WORK" and memname = upcase("&indata.") and name like "month_%";
	...
	proc sql;
		...
		select count(*) as &indata._rec_count, count(distinct bene_id) as &indata._distinct_bene_ids
		from &indata.;
...
%mend;
/*****************************************************************************
%data_cleaning(indata=person_level);/*keep obs missing key vars for review*/
