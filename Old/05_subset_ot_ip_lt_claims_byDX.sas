...
/*macro to get ot claim level data*/
%macro create_ot_extract(indata, outdata);
	proc sql; 
		...
		from &indata.
		where BENE_ID in (select distinct BENE_ID from space.id_pop_25feb2019) and TYPE_CLM_CD="1" and MAX_TOS in (1,8,12,11,16) or PLC_OF_SRVC_CD = 23 and 
	...
	%do i=1 %to 56;
		%if &i. ne 3 and &i. ne 7 and &i. ne 14 and &i. ne 43 and &i. ne 52 %then /*FIPS codes 3,7,14, 42, and 52 do not exist*/
			%do;
				...
				%create_ot_extract(indata=stateot.Maxdata_&state._ot_2012, outdata=ot_sub_&state.);
			%end;
	%end;
%mend;
%do_states();
...
%create_ot_extract(indata=stateot.Maxdata_ca_ot_2012_001, outdata=ot_sub_ca1); 
%create_ot_extract(indata=stateot.Maxdata_ca_ot_2012_002, outdata=ot_sub_ca2);
%create_ot_extract(indata=stateot.Maxdata_ny_ot_2012_001, outdata=ot_sub_ny1);
%create_ot_extract(indata=stateot.Maxdata_ny_ot_2012_002, outdata=ot_sub_ny2);
%create_ot_extract(indata=stateot.Maxdata_tx_ot_2012_001, outdata=ot_sub_tx1);
%create_ot_extract(indata=stateot.Maxdata_tx_ot_2012_002, outdata=ot_sub_tx2);
...
proc sql;
	create table ot_bene_svcdx_svctype as
	select
		bene_id, recipno, state_cd, svc_dx, service_type, 
		SUM(MDCD_PYMT_AMT) AS mdcd_pymt_amt,
		SUM(QTY_SRVC_UNITS) AS QTY_SRVC_UNITS,
		SUM(SRVC_DAYS) AS SRVC_DAYS,
		SUM(CLM_CNT) AS CLM_CNT
	 from ot_cdpsclaims
	 group by bene_id, recipno, state_cd, svc_dx, service_type;
quit;

proc sql;
	select count(*) as all_count,
		(select count(*) from (select distinct * from ot_bene_svcdx_svctype)) as dist_count
	from ot_bene_svcdx_svctype;
quit;

/*macro to pull cdps claims for the other files, which are not split up by state*/
%macro pull_cdps_claims(filetypeshort,svctype,maxtos);
	/*get cpds claims from file*/
	proc sql; 
		create table &filetypeshort._cdpsclaims as
		select BENE_ID, STATE_CD, DIAG_CD_1, MAX_TOS, MDCD_PYMT_AMT, SRVC_BGN_DT, SRVC_END_DT,
			. as QTY_SRVC_UNITS, /*NA for IP & LT file*/
			1 as CLM_CNT,
			SRVC_END_DT-SRVC_BGN_DT+1 as SRVC_DAYS,
			&svctype. as service_type,
			case when DIAG_CD_1 in (&asth.) then "ASTH"
				when DIAG_CD_1 in (&hypert.) then "HYPT"
				when DIAG_CD_1 in (&canc.) then "CANC"
				when DIAG_CD_1 in (&psych.) then "PSYC"
		...
		where BENE_ID in (select distinct BENE_ID from space.id_pop_25feb2019) and TYPE_CLM_CD="1" and MAX_TOS in &maxtos. and 
			(DIAG_CD_1 in (&asth.) or DIAG_CD_1 in (&hypert.) or DIAG_CD_1 in (&canc.) or DIAG_CD_1 in (&psych.) or DIAG_CD_1 in (&preg.) or DIAG_CD_1 in (&diab.));
	quit;
	/*create bene - svc_dx - svc_type level file*/
	proc sql;
		create table &filetypeshort._bene_svcdx_svctype as
		select
			bene_id, state_cd, svc_dx, service_type,catx('_',STATE_CD,BENE_ID) as recipno,  
			SUM(MDCD_PYMT_AMT) AS mdcd_pymt_amt,
			SUM(QTY_SRVC_UNITS) AS QTY_SRVC_UNITS,
			SUM(SRVC_DAYS) AS SRVC_DAYS,
			SUM(CLM_CNT) AS CLM_CNT
		 from &filetypeshort._cdpsclaims
		 group by bene_id, state_cd, svc_dx, service_type;
	quit;
%mend;

%pull_cdps_claims(filetypeshort=ip,svctype="IP_HOSP",maxtos=(1,8,12,11,16));
%pull_cdps_claims(filetypeshort=lt,svctype="NURS_FAC",maxtos=(7));

proc sql;
	create table space.bene_svc_cdpsclaims_&space_name. as
	select * 
	from ot_bene_svcdx_svctype
	union
	select *
	from ip_bene_svcdx_svctype
	union 
	select *
	from lt_bene_svcdx_svctype;
quit;


proc sql;
	select count(*) as all_count,
		(select count(*) from (select distinct * from space.bene_svc_cdpsclaims_&space_name.)) as dist_count
	from space.bene_svc_cdpsclaims_&space_name.;
quit;
