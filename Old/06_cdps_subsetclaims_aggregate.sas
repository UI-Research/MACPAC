...
%let indata_max = space.id_pop_14feb2019; /*input data file from 01_studypop_analyticfile*/
%let year = 2012;
			...
			%if &i. ne 3 and &i. ne 7 and &i. ne 14 and &i. ne 43 and &i. ne 52 %then /*FIPS codes 3,7,14, 42, and 52 do not exist*/
				%do;
					...
					proc sql;
						...
						select *
						from scores.cdps_&state. a inner join space.max_cdpsclaims b
						on a.recipno = b.recipno;
					quit;
					proc datasets nolist;
					   append base=space.cdps_allst data=temp_&state.;
					run;
				%end;
		%end;
	%mend;
...
%mend;
%collapse(indata=max_msa_2012,collapse_on=%str(st_msa),outdata=space.cdps_msa_collapse);
...
%add_vars(collapsed_dat=msa_collapse,collapsed_on=st_msa,orig_dat=ahrf_aggre, outdata=space.cdps_msa_arhfvars);
...
%mend;
%get_stats(indata=max_2012_msa_join,indata_collapsed=msa_arhfvars,orig_data=ahrf_msa_xwalk, collapsevar=st_msa,outdata=out.subset_msa_2012_&space_name.);
