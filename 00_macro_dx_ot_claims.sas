...
%let carel = "401","4010","4011", "4019", "402", "4020", "40200", "4021", "40210", "4029", "40290", 
...
%let canl = "140", "1400", "1401", "1403", "1404", "1405", "1406", "1408", "1409", "141", "1410", "1411", 
...
%let psyl = "290", "2900", "2901", "29010", "29011", "29012", "29013", "2902", "29020", "29021", "2903", "2904", 
...
%let prgcmp = "630", "631", "6310", "6318", "632", "633", "6330", "63300", "63301", "6331", "63310", "63311", "6332", 
			...
			"V276", "V277", "V279";
%let dia2l = "24900", "24901", "24980", "24981", "24990", "24991", "250", "2500", "25000", "25002", "2507", "25070", 
...
%macro pull_ot_claims(pop_data,in_max,cdps_flag,outdata);
	/*get cpds claims from file*/
	proc sql; 
		create table ot_cdpsclaims as
		select BENE_ID, STATE_CD, DIAG_CD_1, MAX_TOS, MDCD_PYMT_AMT, QTY_SRVC_UNITS,
			1 as CLM_CNT,
			...
			case when DIAG_CD_1 in (&pula.) then "ASTH"
				when DIAG_CD_1 in (&carel.) then "HYPT"
				when DIAG_CD_1 in (&canl.) then "CANC"
				when DIAG_CD_1 in (&psyl.) then "PSYC"
				when DIAG_CD_1 in (&prgcmp.) then "PREG" 
				when DIAG_CD_1 in (&dia2l.) then "DIAB"
				end as svc_dx
		from &in_max.
...
proc sql;
	create table limited_cdpspop_dxclms_ot as
	select distinct *
	from all_limited_cdpspop_dxclms_ot;
quit;
