...
%let indata=space.temp_personlevel_geofx; 
proc sql;
	create table bad_geo as
	select *
	from &indata.
	where substr(st_msa,1,2) ne state_cd;
quit;
proc freq data=bad_geo;
	tables st_msa*zip_fx*state_cd*zipcode*replaced_zip/list missing;
run;
proc freq data=bad_geo;
	tables st_msa*zip_fx*county*st_cnty*state_cd*zipcode*replaced_cnty*replaced_zip/list missing;
run;
