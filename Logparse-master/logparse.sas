...
%logparse(saslog=P:\\MCD-SPVR\programs\02_Assemble_Data_Files\sandbox\05_subset_ot_ip_lt_claims_byDX.log,
			outds=logparse, system= OTH, pdsloc = NO,append=NO);
proc print data=logparse;run;

proc sql;
	select sum(realtime) format=time.,  sum(cputime) format=time.
	from logparse;
quit;

%logparse(saslog=P:\MCD-SPVR\log\01_popfile_02NOV19.log,
			outds=log01, system= OTH, pdsloc = NO,append=NO);
%logparse(saslog=P:\MCD-SPVR\log\02_geofix_02NOV19.log,
			outds=log02, system= OTH, pdsloc = NO,append=NO);
%logparse(saslog=P:\MCD-SPVR\log\03_studypop_finaltables_02NOV19.log,
			outds=log03, system= OTH, pdsloc = NO,append=NO);
%logparse(saslog=P:\MCD-SPVR\log\05_addcdps_04NOV19.log,
...
proc sql;
	title "Step 1";
	select sum(realtime) format=TIME. as old_realtime, sum(cputime) format=TIME. as old_cputime
	from log01;
	title "Step 2";
	select sum(realtime) format=TIME. as old_realtime, sum(cputime) format=TIME. as old_cputime
	from log02;
