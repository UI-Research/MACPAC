/*******************************************************************************************************************/ 
/*	Purpose: Pull claims with primary diagnosis for select CDPS chronic conditions subpopulations - OT claims for CA, NY, TX  	
...
%macro prod();
	options obs=MAX;
	/*Log*/
	proc printto print="P:\MCD-SPVR\log\subset_canytx_claims_byDX_&sysdate..lst"
	               log="P:\MCD-SPVR\log\subset_canytx_claims_byDX_&sysdate..log" NEW;
	run;
%mend;
...
%create_ot_extract(indata=stateot.Maxdata_ca_ot_2012_001, outdata=one_ot_sub_ca); 
%create_ot_extract(indata=stateot.Maxdata_ca_ot_2012_002, outdata=two_ot_sub_ca);
%create_ot_extract(indata=stateot.Maxdata_ny_ot_2012_001, outdata=one_ot_sub_ny);
%create_ot_extract(indata=stateot.Maxdata_ny_ot_2012_002, outdata=two_ot_sub_ny);
%create_ot_extract(indata=stateot.Maxdata_tx_ot_2012_001, outdata=one_ot_sub_tx);
%create_ot_extract(indata=stateot.Maxdata_tx_ot_2012_002, outdata=two_ot_sub_tx);


data ot_cdpsclaims_canytx;
	set one_ot_sub: two_ot_sub:;
	recipno=catx ('_',STATE_CD,BENE_ID); 
run;

proc sql;
	create table space.ot_bene_svc_canytx as
	select
	 ...
	 from ot_cdpsclaims_canytx
	 group by bene_id, recipno, state_cd, svc_dx, service_type;
quit;

proc sql;
	select count(*) as all_count,
		(select count(*) from (select distinct * from space.ot_bene_svc_canytx)) as dist_count
	from space.ot_bene_svc_canytx;
quit;
